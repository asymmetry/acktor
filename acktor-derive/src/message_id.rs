...
pub fn expand(ast: &syn::DeriveInput) -> TokenStream {
    ...
    if let Some(value) = custom_id {
        // user-supplied id: skip StableId entirely
        return quote! {
    ...
    // delegate to TYPE_ID; mirror the bounds emitted by the StableId derive so the
    // MessageId impl can name `Self::TYPE_ID` for the same generic parameters
...
mod tests {
    ...
    fn test_type_generics() {
        ...
        // T: StableId appears in both impls' where-clauses
    ...
    fn test_custom_id() {
        ...
        // no StableId impl is emitted when a custom id is supplied
        ...
        assert!(out.contains("impl < T > :: acktor :: MessageId for Wrap < T >"));
        // no auto-added StableId bound on T
