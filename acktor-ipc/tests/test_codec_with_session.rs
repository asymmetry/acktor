...
#[tokio::test]
async fn test_codec_with_session() {
    ...
    let (address, join_handle) = Probe.run("probe").expect("probe run");
    let expected_index =
    ...
    for signal in [Signal::Stop, Signal::Terminate] {
        let bytes = signal.encode_to_bytes(None).expect("encode Signal");
        assert_eq!(Signal::decode(bytes, None).expect("decode Signal"), signal);
    }
    assert!(registry.is_empty());

    // CronSignal — no context, no registry interaction.
    for signal in [CronSignal::Pause, CronSignal::Resume] {
        let bytes = signal.encode_to_bytes(None).expect("encode CronSignal");
        assert_eq!(
            CronSignal::decode(bytes, None).expect("decode CronSignal"),
            signal
        );
    }
    assert!(registry.is_empty());
    ...
    assert!(registry.is_empty());
    let bytes = address
        .encode_to_bytes(Some(&encode_ctx))
        .expect("encode Address");
    let debug_str = format!("{:?}", registry);
    assert_eq!(debug_str, "[6]");
    verify_registry();
    let decoded = RemoteAddress::decode(bytes, Some(&decode_ctx)).expect("decode Address");
    ...
    verify_registry();
    let decoded: Recipient<Ping> = RemoteAddress::decode(buf, Some(&decode_ctx))
    ...
    let bytes = set
        .encode_to_bytes(Some(&encode_ctx))
        .expect("encode Supervisor::Set");
    verify_registry();
    match <Supervisor<Probe>>::decode(bytes, Some(&decode_ctx)).expect("decode Supervisor::Set") {
    ...
    let bytes = register
        .encode_to_bytes(Some(&encode_ctx))
        .expect("encode Observer::Register");
    verify_registry();
    match <Observer<Ping> as Decode>::decode(bytes, Some(&decode_ctx))
        .expect("decode Observer::Register")
    {
    ...
    let bytes = unregister
        .encode_to_bytes(Some(&encode_ctx))
        .expect("encode Observer::Unregister");
    verify_registry();
    match <Observer<Ping> as Decode>::decode(bytes, Some(&decode_ctx))
        .expect("decode Observer::Unregister")
    {
    ...
    let bytes = warn
        ...
        .expect("encode Warn");
    verify_registry();
    match RemoteSupervisionEvent::decode(bytes, Some(&decode_ctx)).expect("decode Warn") {
    ...
    let bytes = terminated
        .encode_to_bytes(Some(&encode_ctx))
        .expect("encode Terminated");
    verify_registry();
    match RemoteSupervisionEvent::decode(bytes, Some(&decode_ctx)).expect("decode Terminated") {
    ...
    verify_registry();
    match RemoteSupervisionEvent::decode(bytes, Some(&decode_ctx)).expect("decode Panicked") {
    ...
    state
        .encode(&mut bytes, Some(&encode_ctx))
        .expect("encode State");
    let bytes = bytes.freeze();
    verify_registry();
    match RemoteSupervisionEvent::decode(bytes, Some(&decode_ctx)).expect("decode State") {
