...
impl Handler<SupervisionEvent<TestActor>> for Watcher {
    ...
    async fn handle(
        ...
        match msg {
            SupervisionEvent::Warn(_, error) => self.events.push(Event::Warn(error.to_string())),
            SupervisionEvent::Terminated(_, error) => self
                .events
                .push(Event::Terminated(error.map(|e| e.to_string()))),
            _ => {}
        }
    }
}
...
async fn test_post_stop_error() -> Result<()> {
    ...
    // if handler returns error, we should receive the handler error instead of the post_stop error
